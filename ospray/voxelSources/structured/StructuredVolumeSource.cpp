...
namespace ospray {
  namespace impi {
    namespace structured {
      ...
      void   StructuredVolumeSource::getActiveVoxels(std::vector<VoxelSource::VoxelRef> &activeVoxels, float isoValue) const 
      {
        volume->filterAllVoxelsThatOverLapIsoValue((std::vector<structured::VoxelRef>&)activeVoxels,isoValue);
      }
      
      /*! compute world-space bounds for given voxel */
      box3fa StructuredVolumeSource::getVoxelBounds(const VoxelSource::VoxelRef voxelRef) const 
      {
      ...
      Impi::Voxel  StructuredVolumeSource::getVoxel(const VoxelSource::VoxelRef voxelRef) const 
      {
        Impi::Voxel voxel;
        const vec3i idx = ((structured::VoxelRef &)voxelRef).asVec3i();
        const vec3f lo = vec3f(idx)*scaleDims;
