...
namespace ospray {
  namespace impi {
	...
	: reconMethod(ospcommon::utility::getEnvVar<std::string>
		      ("IMPI_AMR_METHOD").value_or("octant")),
	  storeMethod(ospcommon::utility::getEnvVar<std::string>
		      ("IMPI_AMR_STORAGE").value_or("active")),
	  amrVolumePtr(amr)
      {
	/* debug */
	...
	/* get AMR volume pointer */
        if (!amr) throw std::runtime_error("Empty amr volume");	  
...
namespace ospray {
  namespace impi {
	...
	const box3fa box(coordinate, coordinate + cellwidth);
      ...
      /*! compute world-space bounds for given voxel */
      box3fa TestOctant::getVoxelBounds_active(const VoxelRef voxelRef) 
	const 
      {
	...
	delete[] leafActiveOctants;
        ...
        std::cout << "Done Init Octant Value! " << voxels.size() 
		  << std::endl;
      }

      /*! compute active voxels (called in Impi.cpp file) */
      void TestOctant::getActiveVoxels_active(std::vector<VoxelRef> 
					      &activeVoxels, 
					      float isoValue) const
      {
	...
	voxel.bounds.upper = voxel.bounds.lower + cellwidth;
      ...
      /*! compute active voxels (called in Impi.cpp file) */
      void TestOctant::getActiveVoxels_none(std::vector<VoxelRef> 
					    &activeVoxels, 
					    float isoValue) const
      {
	//
