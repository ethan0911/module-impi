MOVED TO volumeSources/structured
...
//         VoxelRef(const vec3i &idx)
//           : x(idx.x), y(idx.y), z(idx.z)
//         {}
      
//         inline vec3i asVec3i() const { return vec3i(x,y,z); }
...
//         /* query get the eight corner voxels (in float) of given voxel */
//         inline Voxel getVoxel(const vec3i &idx) const { Voxel c; getVoxel(c,idx); return c; }
...
//       /*! build list of all voxels that fulfill the given filter lambda */
//       template<typename Lambda>
//       inline void LogicalVolume::filterVoxels(std::vector<VoxelRef> &out, Lambda filter)
//       {
