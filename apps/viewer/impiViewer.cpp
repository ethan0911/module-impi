...
namespace ospray {
  ...
  namespace impi {
    ...
    struct ImpiSGNode : public sg::Geometry
    {
      ...
      void postCommit(sg::RenderContext &ctx)
      {
        auto materialListNode =
            child("materialList").nodeAs<sg::MaterialList>();
    ...
    std::shared_ptr<sg::Importer> importObject2World(sg::Node &renderer,
                                                     std::string fileName)
    {
      ...
      auto &transform = world.createChild("transform_" + fileName, "Transform");
    ...
    std::shared_ptr<sg::Node> DataParser(
      ...
      if (dataString == "landingGear") {
        ...
        if (args.inputFiles.size() != 2) {
          throw std::runtime_error(
              "missing input arguments for 'landingGear' dataset");
        }
        // Setup Data States
        ...
        auto landingGearImportNodePtr =
            importObject2World(renderer, args.inputFiles[1]);
    ...
    extern "C" int main(int ac, const char **av)
    {
      ...
      if (!args.inputFiles.empty()) {
        ...
        auto amrVolSGNodePtr =
            importerNodePtr->childByType("AMRVolume");  // foundNode->second;
        (*amrVolSGNodePtr)["visible"] =
            false;  // AMR volume transfer function is buggy ?
        ...
        auto objNode =
            DataParser(dataString, args, renderer, world, impiGeometryNode);

        impiGeometryNode->createChild("isoValue", "float", dataState.isoValue);
        impiGeometryNode->createChild(
            "amrDataPtr",
            "OSPObject",
      ...
      auto &lights = renderer["lights"];
      {
        ...
        std::cout << "disDir: " << dataState.disDir.x << " "
                  << dataState.disDir.y << " " << dataState.disDir.z
                  << std::endl;
      ...
      // XXX SG is too restrictive: OSPRay cameras accept non-normalized
      // directions
      ...
      if (renderer["camera"].hasChild("focusdistance")) {
        renderer["camera"]["focusdistance"] =
            length(viewPort.at - viewPort.from);
      }
